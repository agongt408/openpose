...
def keypoints(img_path, shape=input_shape, train=False, preprocess=input_preprocess, r=input_r, keypoint=None):
    if img_path.find('/') > -1:
    ...
    if keypoint is None:
        ...
        for person in range(len(data['people'])):
            x_key = data['people'][person]['pose_keypoints'][BODY_PARTS.index(keypoint) * 3] * shape[1] / 64.0
            y_key = data['people'][person]['pose_keypoints'][BODY_PARTS.index(keypoint) * 3 + 1] * shape[0] / 128.0
            c_key = data['people'][person]['pose_keypoints'][BODY_PARTS.index(keypoint) * 3 + 2]
            if not (x_key == 0 and y_key == 0):
                htmp = np.maximum(htmp, makeGaussian(shape, r=r, center=(x_key, y_key)))
...
def output_batch_generator(files_dict, labels, P=P_param, K=K_param, shape=input_shape,
                            preprocess=input_preprocess, overlay=input_overlay, r=input_r, keypoint=None):
    print preprocess
    print overlay
    while True:
        im_batch = []
        ...
        for p in range(len(idt_choice)):
            ...
            for k in k_choice:
                ...
                if overlay:
                    cam_batch.append(keypoints(path, shape=(128,64), train=True, preprocess=preprocess, r=r, keypoint=keypoint).tolist())
                im_batch.append(imread_scale(path, shape=shape, preprocess=preprocess).tolist())
        if overlay:
            yield {'input_1' : np.array(im_batch),
                   'input_cam' : np.array(cam_batch)}, {'dense_3' : np.zeros((P*K, input_output_dim))}
        else:
        ...
        for f in files_dict[idt]:
            ...
            if overlay:
                if overlay_eval:
                    ...
                    input_dict = {'input_1' : img.reshape(1,shape[0],shape[1],3),
            ...
            for f in f_choice:
                ...
                if overlay:
                    if overlay_eval:
                        ...
                        input_dict = {'input_1' : query_img.reshape(1,shape[0],shape[1],3),
                                    'input_cam' : cam.reshape(1,128,64)}
                    else:
                        input_dict = {'input_1' : query_img.reshape(1,shape[0],shape[1],3),
                                    'input_cam' : np.ones((1,128,64))}
