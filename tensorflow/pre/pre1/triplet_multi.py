...
def overlay_keypoints(img_path, shape=input_shape, train=False,
                        preprocess=input_preprocess, fwhm=20, region='whole'):
    if img_path.find('/') > -1:
    ...
    if region == 'front':
        for person in range(len(keypoints)):
            if keypoints[person]['x'][2] < keypoints[person]['x'][5]:
                for k in range(len(keypoints[person]['x'])):
                    x_key = keypoints[person]['x'][k]
                    y_key = keypoints[person]['y'][k]
                    if not (x_key == 0 and y_key == 0):
                        htmp = np.maximum(htmp, makeGaussian(shape, fwhm=fwhm, center=(x_key, y_key)))
    elif region == 'back':
        for person in range(len(keypoints)):
...
def output_batch_generator(files_dict, labels, P=P_param, K=K_param, shape=input_shape,
                            preprocess=input_preprocess, overlay=input_overlay, region='whole'):
    print preprocess
    print overlay
    while True:
        batch = []
            ...
            for k in k_choice:
                ...
                if overlay:
                    batch.append(overlay_keypoints(path, shape=shape, train=True,
                                    preprocess=preprocess, region=region).tolist())
                else:
