...
def output_batch_generator(files_dict, labels, P=P_param, K=K_param, shape=input_shape,
    ...
    while True:
        ...
        if overlay:
            yield {'input_1_b' : np.array(im_batch),
                    'input_1_n' : np.array(im_batch), 'input_cam_n' : np.ones((P*K,128,64)),
                    'input_1_RS' : np.array(im_batch), 'input_cam_RS' : np.ones((P*K,128,64)),
                    'input_1_LS' : np.array(im_batch), 'input_cam_LS' : np.ones((P*K,128,64)),
                    'input_1_RH' : np.array(im_batch), 'input_cam_RH' : np.ones((P*K,128,64)),
                    'input_1_LH' : np.array(im_batch), 'input_cam_LH' : np.ones((P*K,128,64))},{'final_output' : np.zeros((P*K, input_output_dim))}
        else:
        ...
        for f in files_dict[idt]:
            ...
            if overlay:
                if overlay_eval:
                    ...
                    input_dict = {'input_1_b' : img.reshape(1,shape[0],shape[1],3),
                    'input_1_n' : img.reshape(1,shape[0],shape[1],3), 'input_cam_n' : np.ones((1,128,64)),
                    'input_1_RS' : img.reshape(1,shape[0],shape[1],3), 'input_cam_RS' : np.ones((1,128,64)),
                    'input_1_LS' : img.reshape(1,shape[0],shape[1],3), 'input_cam_LS' : np.ones((1,128,64)),
                    'input_1_RH' : img.reshape(1,shape[0],shape[1],3), 'input_cam_RH' : np.ones((1,128,64)),
                    'input_1_LH' : img.reshape(1,shape[0],shape[1],3), 'input_cam_LH' : np.ones((1,128,64))}
            ...
            for f in f_choice:
                ...
                if overlay:
                    if overlay_eval:
                        ...
                        input_dict = {'input_1_b' : query_img.reshape(1,shape[0],shape[1],3),
                    'input_1_n' : query_img.reshape(1,shape[0],shape[1],3), 'input_cam_n' : np.ones((1,128,64)),
                    'input_1_RS' : query_img.reshape(1,shape[0],shape[1],3), 'input_cam_RS' : np.ones((1,128,64)),
                    'input_1_LS' : query_img.reshape(1,shape[0],shape[1],3), 'input_cam_LS' : np.ones((1,128,64)),
                    'input_1_RH' : query_img.reshape(1,shape[0],shape[1],3), 'input_cam_RH' : np.ones((1,128,64)),
                    'input_1_LH' : query_img.reshape(1,shape[0],shape[1],3), 'input_cam_LH' : np.ones((1,128,64))}
