...
def output_batch_generator(files_dict, labels, P=P_param, K=K_param, shape=input_shape,
    ...
    while True:
        ...
        yield {'input_1_b' : np.array(batch),
               'input_1_f' : np.array(batch),
               'input_1_k' : np.array(batch)}, {'final_output' : np.zeros((P*K, 128))}
    ...
    for idt in files_dict.keys():
        for f in files_dict[idt]:
            ...
            input_dict = {'input_1_b' : img.reshape(1,shape[0],shape[1],3),
                            'input_1_f' : img.reshape(1,shape[0],shape[1],3),
                            'input_1_k' : img.reshape(1,shape[0],shape[1],3)}
        ...
        for x in range(runs_per_rank):
            ...
            for f in f_choice:
                ...
                input_dict = {'input_1_b' : query_img.reshape(1,shape[0],shape[1],3),
                            'input_1_f' : query_img.reshape(1,shape[0],shape[1],3),
                            'input_1_k' : query_img.reshape(1,shape[0],shape[1],3)}
