...
def makeRect(shape, r=input_r, center=None):
    x = np.arange(0, shape[1], 1, float)
    y = np.arange(0, shape[0], 1, float)[:,np.newaxis]
    ...
    result = cv2.rectangle(np.zeros(shape), (x0-radius,y0-radius),
                (x0+radius,y0+radius), 1, -1).astype(np.float64)
    return result

def create_keypoints(img_path, shape=input_shape, preprocess=input_preprocess,
                    r=input_r, keypoints=None, DATA_ROOT='/home/albert/github/tensorflow/', rect=True):
    if img_path.find('/') > -1:
        root = img_path[len(img_path) - img_path[::-1].index('/'):-4:]
    else:
        root = img_path[0:img_path.index('.')]
        ...
        for person in range(len(data['people'])):
            for k in keypoints:
                ...
                if not (x_key == 0 and y_key == 0):
                    if rect:
                        htmp = np.maximum(htmp, makeRect(shape, r, (x_key, y_key)))
                    else:
...
def output_batch_generator(files_dict, model=None, P=P_param, K=K_param, shape=input_shape,
                            ...
                            crop=False, flip=False, DATA_ROOT='/home/albert/github/tensorflow/', rect=False):
    ...
    while True:
        ...
        if cam_output_dim is not None:
            if 'Neck' in keypoints:
                output_dict['neck_cam_output'] = []
            if 'RHip' in keypoints or 'LHip' in keypoints:
                output_dict['hip_cam_output'] = []
        ...
        for p in range(len(idt_choice)):
            ...
            for k in k_choice:
                ...
                if keypoints is not None and keypoints != 'ones':
                    cam_batch.append(create_keypoints(path, ((128,64)), preprocess,
                                    r, keypoints, DATA_ROOT, rect).tolist())

                if cam_output_dim is not None:
                    if 'Neck' in keypoints:
                        htmp = create_keypoints(path, cam_output_dim, preprocess, r, ['Neck'], DATA_ROOT, rect)
                        if htmp.max() != 0.0:
                            htmp = htmp / htmp.sum()
                        output_dict['neck_cam_output'].append(htmp.tolist())

                    if 'RHip' in keypoints or 'LHip' in keypoints:
                        htmp = create_keypoints(path, cam_output_dim, preprocess, r, ['RHip', 'LHip'],
                                                DATA_ROOT, rect)
                        if htmp.max() != 0.0:
                            htmp = htmp / htmp.sum()
                        output_dict['hip_cam_output'].append(htmp.tolist())
                im = None
                if crop:
                    ...
                    if rect:
                        im *= np.tile(create_keypoints(path, shape, preprocess, 0.3, ['Neck']).reshape((shape[0], shape[1], 1)), [1,1,3])
        ...
        if cam_output_dim is not None:
            if 'Neck' in keypoints:
                output_dict['neck_cam_output'] = np.array(output_dict['neck_cam_output'])
            if 'RHip' in keypoints or 'LHip' in keypoints:
                output_dict['hip_cam_output'] = np.array(output_dict['hip_cam_output'])
