    ...
    while True:
        ...
        for p in range(len(idt_choice)):
            ...
            for k in k_choice:
                ...
                if overlay:
                    cam_batch.append(keypoints(path, shape=shape, train=True, preprocess=preprocess, r=r).tolist())
...
def evaluate_rank(model, files_dict, files_arr, rank=[1,5,20], train=True,
                    ...
                    overlay=input_overlay, preprocess=input_preprocess, r=input_r):
    ...
    for idt in files_dict.keys():
        for f in files_dict[idt]:
            ...
            if overlay:
                cam = keypoints(f, shape=shape, train=train, preprocess=preprocess, r=r)
                input_dict = {'input_im' : img.reshape(1,shape[0],shape[1],3),
                                'input_cam' : cam.reshape(1,shape[0],shape[1],3)}
    ...
    for r in rank:
        ...
        for x in range(runs_per_rank):
            ...
            for f in f_choice:
                ...
                if overlay:
                    cam = keypoints(files_arr[f][0], shape=shape, train=train, preprocess=preprocess, r=r)
...
def evaluate_dist(model, generator, n_pairs=1000, train=False, shape=input_shape,
    ...
    for t in range(n_pairs):
        ...
        for i in range(2):
            ...
            if overlay:
                cam = keypoints(pair[i], shape=shape, train=train, preprocess=preprocess, r=r)
                input_dict = {'input_im' : img.reshape(1,shape[0],shape[1],3),
                                'input_cam' : cam.reshape(1,shape[0],shape[1],3)}
...
def get_score(model, files_dict, files_arr, rank_iter=1000, runs_per_rank=3,
    ...
    pos_distance = evaluate_dist(model, pos_generator, train=train,
                                preprocess=preprocess, overlay=overlay, shape=shape, r=r)
